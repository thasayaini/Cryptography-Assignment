...
class CryptoApp:
    def __init__(self, root):
        ...
        self.btn_generate_keys = tk.Button(
            left_frame,
            text="Generate RSA Keys & Exchange AES Key",
        ...
        self.rsa_text = scrolledtext.ScrolledText(
            left_frame, height=20, width=50, bg="#ecf0f1"
        )
        self.rsa_text.pack()
        ...
        self.btn_tamper = tk.Button(
            right_frame,
            text="Tamper Encrypted Data",
            command=self.tamper_encrypted_data,
            bg="#f39c12",
            fg="white",
            font=("Arial", 10, "bold"),
        )
    ...
    def generate_rsa_keys(self):
        ...
        self.rsa_text.insert(
            tk.END, f"Public Key (e, n): ({self.rsa_A.e}, {self.rsa_A.n})\n"
        )
        self.rsa_text.insert(
            tk.END, f"Private Key (d, n): ({self.rsa_A.d}, {self.rsa_A.n})\n\n"
        )

        self.rsa_text.insert(tk.END, "Person B (RSA_B) Keys:\n")
        self.rsa_text.insert(
            tk.END, f"Public Key (e, n): ({self.rsa_B.e}, {self.rsa_B.n})\n"
        )
        self.rsa_text.insert(
            tk.END, f"Private Key (d, n): ({self.rsa_B.d}, {self.rsa_B.n})\n\n"
        )

        self.key_exchange = KeyExchange(self.rsa_A, self.rsa_B)
        self.aes_key, encrypted_aes_key, decrypted_aes_key = (
            self.key_exchange.generate_and_exchange_aes_key()
        )

        self.aes_cipher = AES_Cipher(decrypted_aes_key.hex())

        self.rsa_text.insert(
            tk.END, f"Generated AES Key (Random): {self.aes_key.hex()}\n"
        )
        self.rsa_text.insert(
            tk.END, f"Encrypted AES Key (Sent to B): {encrypted_aes_key}\n"
        )
        self.rsa_text.insert(
            tk.END, f"Decrypted AES Key (By B): {decrypted_aes_key.hex()}\n"
        )
    ...
    def encrypt_message(self):
        ...
        if plaintext:
            ...
            self.btn_tamper.config(
                state=tk.NORMAL, bg="#f39c12"
            )  # Enable tamper button
    ...
    def tamper_encrypted_data(self):
        ...
        if encrypted_data:
            ...
            self.btn_tamper.config(
                state=tk.DISABLED, bg="#d3d3d3"
            )  # Disable tamper button
        ...
        if encrypted_text:
            try:
                ...
                if not decrypted_text:
                    self.decrypted_text.insert(0, "incomprehensible")
            except Exception:
                try:
                    ...
                    for i in range(0, len(encrypted_msg), 16):
                        ...
                        decrypted_block = self.aes_cipher.decrypt_block(block)
                        plaintext_block = self.xor_bytes(
                    ...
                    garbled_text = plaintext.decode(errors="replace")
                    self.decrypted_text.delete(0, tk.END)
                    self.decrypted_text.insert(0, garbled_text)
                    if not garbled_text.strip():
                        self.decrypted_text.insert(0, "incomprehensible")
                except Exception as e:
                    # If all attempts fail, show corrupted text as best as possible
                    self.decrypted_text.delete(0, tk.END)
                    self.decrypted_text.insert(0, "Decryption Failed")
